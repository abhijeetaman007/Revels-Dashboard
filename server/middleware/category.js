...
const isCulturalCategory = async(req,res) =>{
    try{
        let user = req.reqeustUser;
        if(user.categoryId.type == "CULTURAL")
            next()
...
const isSysAdmin = async(req,res) =>{
    try{
        let user = req.reqeustUser;
        if(user.categoryId.type == "SUPPORTING" && user.categoryId.category == "SYSADMIN")
            next()
        return res.status(403).send({msg:'Access Denied',success:false})
    }   
    catch(err)
...
const isOperation = async(req,res) =>{
    try{
        let user = req.reqeustUser;
        if(user.categoryId.type == "SUPPORTING" && user.categoryId.category == "OPERATION" )
            next()
        return res.status(403).send({msg:'Access Denied',success:false})
    }   
    catch(err)
...
const isVigilance = async(req,res) =>{
    try{
        let user = req.reqeustUser;
        if(user.categoryId.type == "SUPPORTING" && user.categoryId.category == "VIGILANCE" )
            next()
        return res.status(403).send({msg:'Access Denied',success:false})
    }   
    catch(err)
