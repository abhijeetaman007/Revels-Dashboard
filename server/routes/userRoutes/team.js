...
const teamRegister = async (req, res) => {
  try {
    ...
    if (flag == 0) {
      return res
        .status(400)
        .json({ msg: 'Please register to event first then create a team' });
    }
    //teams of that event
...
const joinTeam = async (req, res) => {
  try {
    ...
    if (flag == 0) {
      return res
        .status(400)
        .json({ msg: 'Please register to event first then join a team' });
    }
    //teams of that event
    const teams = await Team.find({ event: event._id });
    ...
    if (!team)
      return res.status(404).json({ msg: 'Team not found with given code' });
    //check if team full
